...
$(document).ready(function(){
    ...
    $(document).on('click', '.index_details, .index_edit, .index_del', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (details || del) {
                    // entry information
                    $('.modal_entry_date span').text(datetimeToDateFormat(data['entry']['date']));
                    if (data['entry']['status'] == 1) {
                        ...
                        }).addClass('badge-soft-primary ').text('Paid');
                    } else if (data['entry']['status'] == 2) {
                        $('.modal_entry_status').removeClass(function (index, className) {
                            return (className.match (/(^|\s)badge-soft-\S+/g) || []).join(' ');
                        }).addClass('badge-soft-secondary ').text('Open');
                    } else if (data['entry']['status'] == 3) {
                        $('.modal_entry_status').removeClass(function (index, className) {
                            return (className.match (/(^|\s)badge-soft-\S+/g) || []).join(' ');
                        }).addClass('badge-soft-danger ').text('In negotiation');
                    } else if (data['entry']['status'] == 4) {
                        $('.modal_entry_status').removeClass(function (index, className) {
                            return (className.match (/(^|\s)badge-soft-\S+/g) || []).join(' ');
                        }).addClass('badge-soft-success ').text('Received');
                    } else {
                        $('.modal_entry_status').removeClass(function (index, className) {
                            return (className.match (/(^|\s)badge-soft-\S+/g) || []).join(' ');
                        }).addClass('badge-soft-warning ').text('Booked');
                    }
                    $('.modal_entry_amount span').text(formatCurrency(data['entry']['amount']));
                    if (data['entry']['description']) {
                        $('.modal_entry_description span').text(data['entry']['description']);
                        $('.modal_entry_description').removeClass('d-none');
                        $('.modal_entry_amount').removeClass('mb-4').addClass('mb-2')
                    } else {
                        $('.modal_entry_description').addClass('d-none');
                        $('.modal_entry_amount').removeClass('mb-2').addClass('mb-4');
                    }
                    
                    // category information
                    $('.modal_category span').text(data['subcategory']['category_name']);
                    if (data['subcategory']['category_type'] == 1) {
                        $('.modal_category_type').removeClass(function (index, className) {
                            return (className.match (/(^|\s)badge-soft-\S+/g) || []).join(' ');
                        }).addClass('badge-soft-success ').text('Income');
                    } else {
                        $('.modal_category_type').removeClass(function (index, className) {
                            return (className.match (/(^|\s)badge-soft-\S+/g) || []).join(' ');
                        }).addClass('badge-soft-danger ').text('Expense');
                    }
                    $('.modal_subcategory span').text(data['subcategory']['name']);

                    // beneficiary information
                    $('.modal_beneficiary_category span').text(data['beneficiary']['category']);
                    $('.modal_beneficiary_name span').text(data['beneficiary']['name']);

                    // financial information
                    $('.modal_bank span').text(data['financial_account']['bank']);
                    $('.modal_branch span').text(data['financial_account']['branch']);
                    $('.modal_account span').text(data['financial_account']['account']);

                    if (!isEmpty(data['financial_cost_center'])) {
                        if ($('.modal_account').hasClass('mb-2')) {
                            $('.modal_account').removeClass('mb-2').addClass('mb-4');
                        }
                        $('.cost_center_modal').removeClass('d-none');
                        $('.modal_cost_center span').text(data['financial_cost_center']['name']);
                        if (data['financial_cost_center']['description']) {
                            if ($('.modal_cost_center').hasClass('mb-4')) {
                                $('.modal_cost_center').removeClass('mb-4').addClass('mb-2');
                            }
                            $('.modal_description').removeClass('d-none');
                            $('.modal_description span').text(data['financial_cost_center']['description']);
                            if (isEmpty(data['client'])) {
                                $('.modal_description').removeClass('mb-4').addClass('mb-2');
                            } else {
                                $('.modal_description').removeClass('mb-2').addClass('mb-4');
                            }
                        } else {
                            ...
                            if (isEmpty(data['client'])) {
                                ...
                                $('.modal_cost_center').removeClass('mb-2').addClass('mb-4');
                            }
                        }
                    } else {
                        $('.cost_center_modal').addClass('d-none');
                        if (isEmpty(data['client'])) {
                            $('.modal_account').removeClass('mb-4').addClass('mb-2');
                        } else {
                            $('.modal_account').removeClass('mb-2').addClass('mb-4');
                        }
                    }
                    ...
                    if (!isEmpty(data['client'])) {
                        let noneCounter = 0;
                        $('.client_modal').removeClass('d-none');
                        $('.modal_client span').text(data['client']['name']);
                        $('.modal_region span').text(data['client']['region']);
                        $('.modal_state span').text(data['client']['state']);
                        $('.modal_city span').text(data['client']['city']);
                        $('img.modal_flag').attr('src', '/static/board/images/flags/'+data['client']['flag']);

                        if (data['client']['email']) {
                            $('.modal_email').removeClass('d-none');
                            $('.modal_email span').text(data['client']['email']);
                            extraClientModal();
                        } else {
                        ...
                        if (data['client']['phone']) {
                            ...
                            }).unmask().addClass('mask_phone_'+data['client']['region'].replaceAll(' ', '_').toLowerCase()).text(data['client']['phone']);
                            extraClientModal();
                            initMasks();
                        } else {
                        ...
                        if (data['client']['responsible']) {
                            $('.modal_responsible').removeClass('d-none');
                            $('.modal_responsible span').text(data['client']['responsible']);
                            extraClientModal();
                        } else {
                        ...
                        if (noneCounter == 3) {
                            $('.extra_client_modal').addClass('d-none');
                            $('.modal_city').removeClass('mb-4').addClass('mb-2');
                        }

                        function extraClientModal() {
                            if ($('.extra_client_modal').hasClass('d-none')) {
                                $('.extra_client_modal').removeClass('d-none');
                            }
                    ...
                    $('.entry_val').val(data['entry']['date']);
                    $('.category_modal_edit option[value="'+data['subcategory']['category_value']+'"]').prop('selected', true);

                    $('.subcategory_modal').find('option').remove().end()
                    $(data['subcategory']['list']).each(function(index, value) {
                        $('.subcategory_modal').append(new Option(value['name'], value['slug']));
                    });
                    $('.subcategory_modal option[value="'+data['subcategory']['value']+'"]').prop('selected', true);

                    $('.note_modal').val(data['entry']['description']);

                    $('.beneficiary_modal option[value="'+data['beneficiary']['value']+'"]').prop('selected', true);
                    $('.client_modal option[value="'+data['client']['value']+'"]').prop('selected', true);

                    $('.cost_center_modal option[value="'+data['financial_cost_center']['value']+'"]').prop('selected', true);
                    $('.status_modal option[value="'+data['entry']['status']+'"]').prop('selected', true);

                    $('.account_modal option[value="'+data['financial_account']['value']+'"]').prop('selected', true);
                    $('.amount_modal').val(formatCurrency(data['entry']['amount']).replace('$', ''));
    ...
    $(document).on('click', '.move-day', function() {
        ...
        let renderRange = $('#renderRange').text()
        let month = parseInt(getMonthFromString(renderRange.split('.')[0]))
